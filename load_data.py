...
df_categories = pd.read_csv(f"{CSV_PATH}/categories.csv")  
...
df_categories_db = df_categories.rename(columns={"id": "category_id"})[["category_id", "slug", "name"]]
df_categories_db.to_sql("categories", engine, if_exists="append", index=False,)
...
df_shipping_db = df_shipping[["shipping_regions_id", "region", "shipping_zone", "estimated_days", "country_code"]]
df_shipping_db.to_sql("shipping_regions", engine, if_exists="append", index=False,)
...
df_users_db = df_users.rename(columns={"id": "user_id"})[["user_id", "email", "created_at", "country_code"]]
df_users_db.to_sql("users", engine, if_exists="append", index=False,)
...
df_products_db = df_products.rename(columns={"id": "product_id"})[["product_id", "name", "price", "category_id"]]
df_products_db.to_sql("products", engine, if_exists="append", index=False,)
...
df_pd_db.to_sql("product_details", engine, if_exists="append", index=False,)
...
df_orders_db = df_orders.rename(columns={"id": "order_id", "order_date": "created_at"})[["order_id", "created_at", "total_amount", "user_id"]]
df_orders_db.to_sql("orders", engine, if_exists="append", index=False,)
...
df_items_db = df_items.rename(columns={"id": "order_items_id"})[["order_items_id", "quantity", "unit_price", "order_id", "product_id"]]
df_items_db.to_sql("order_items", engine, if_exists="append", index=False,)
